...
public final class WrapWriteWord implements Function {
	...
	try {
		stream.out.write(
			ByteBuffer.allocate(2).order(ByteOrder.LITTLE_ENDIAN).putShort(element).array());
	} catch (IOException e) {
		e.printStackTrace();
		exception = e;
	}
...
public final class WrapWriteInt implements Function {
	...
	try {
		stream.out.write(
			ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putInt(element).array());
	} catch (IOException e) {
		e.printStackTrace();
		exception = e;
	}
...
public final class WrapWriteFloat implements Function {
	...
	try {
		stream.out.write(
			ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putFloat(element).array());
	} catch (IOException e) {
		e.printStackTrace();
		exception = e;
	}
...
public final class WrapWriteDouble implements Function {
	...
	try {
		stream.out.write(
			ByteBuffer.allocate(4).order(ByteOrder.LITTLE_ENDIAN).putDouble(element).array());
	} catch (IOException e) {
		e.printStackTrace();
		exception = e;
	}
...
public final class WrapReadWord implements Function {
	...
	try {
		...
		element =
			(int) ByteBuffer.wrap(buffer).order(ByteOrder.LITTLE_ENDIAN).asShortBuffer().get(0);
	} catch (Exception e) {
		e.printStackTrace();
		exception = e;
	}
...
public final class WrapFindNextFile implements Function {
	...
	// Return data
	if (findFileHandle != -1
		&& findFileCollection != null
		&& findFileHandle < findFileCollection.size()) {
