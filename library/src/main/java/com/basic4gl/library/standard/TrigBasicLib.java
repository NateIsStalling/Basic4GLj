...
public class TrigBasicLib implements FunctionLibrary {
    ...
    s.put(
        ...
        new FunctionSpecification[] {
          new FunctionSpecification(
              WrapVec3.class,
              new ParamTypeList(
                  BasicValType.VTP_REAL, BasicValType.VTP_REAL, BasicValType.VTP_REAL),
              true,
              true,
              new ValType(BasicValType.VTP_REAL, (byte) 1, (byte) 1, true),
              false,
  ...
  public static void crossProduct(
      float[] v1, int offset1, float[] v2, int offset2, float[] result, int offset3) {
    assertTrue(v1 != null);
    assertTrue(v1 != null);
    assertTrue(result != null);
    ...
    result[3 + offset3] =
        calcW(
            Arrays.copyOfRange(v1, offset1, v1.length), Arrays.copyOfRange(v2, offset2, v2.length));
  }
  ...
  public static void matrixTimesVec(
      float[] m, int offset1, float[] v, int offset2, float[] result, int offset3) {
    assertTrue(m != null);
    assertTrue(v != null);
    assertTrue(result != null);
    for (int y = 0; y < 4; y++) {
  ...
  public static void rotateAxis(float ang, float[] v) {
    ...
    // Precalc sin/cos
    float c = (float) Math.cos(ang * Standard.M_DEG2RAD),
        s = (float) Math.sin(ang * Standard.M_DEG2RAD);
  ...
  static int readVec(TomVM vm, int index, float[] v) {
    ...
    v[3] = 1;
    Data.readArray(
        vm.getData(), index, new ValType(BasicValType.VTP_REAL, (byte) 1, (byte) 1, true), v, size);
  ...
  static boolean readMatrix(TomVM vm, int index, float[] m) {
    ...
    // Read in matrix
    Data.readArray(
        vm.getData(), index, new ValType(BasicValType.VTP_REAL, (byte) 2, (byte) 1, true), m, 16);
    return true;
  }
  ...
  public static final class WrapVec4 implements Function {
    public void run(TomVM vm) {
      ...
      float[] vec4 =
          new float[] {
            vm.getRealParam(4), vm.getRealParam(3), vm.getRealParam(2), vm.getRealParam(1)
          };
  ...
  public static final class OpMatrixMatrix implements Function {
    public void run(TomVM vm) {
      ...
      vm.getReg()
          .setIntVal(Data.fillTempRealArray2D(vm.getData(), vm.getDataTypes(), 4, 4, result));
    }
  }
  ...
  public static final class OpVecPlusVec implements Function {
    public void run(TomVM vm) {
      ...
      vm.getReg()
          .setIntVal(
              Data.fillTempRealArray(vm.getData(), vm.getDataTypes(), Math.max(s1, s2), result));
    }
  }
  ...
  public static final class OpMatrixPlusMatrix implements Function {
    public void run(TomVM vm) {
      ...
      vm.getReg()
          .setIntVal(
              Data.fillTempRealArray2D(
                  vm.getData(), vm.getDataTypes(), 4, 4, Arrays.asList(result)));
    }
  }
  ...
  public final class TrigUnOperatorExtension implements UnaryOperatorExtension {
    ...
    public boolean run(
      ...
      if (regType.get().getVirtualPointerLevel() > 0
  ...
  public final class TrigBinOperatorExtension implements BinaryOperatorExtension {
    ...
    public boolean run(
        ...
        //         OUT: Required type cast before calling function
        Mutable<ValType>
            reg2Type, // IN: Current type in second register (operation is reg2 OP reg1, e.g reg2 +
      ...
      if (regType.get().getVirtualPointerLevel() > 0
          || reg2Type.get().getVirtualPointerLevel() > 0) {
        return false;
      }

      // Validate data types. We will only work with ints and reals
      if (regType.get().basicType != BasicValType.VTP_REAL
          && regType.get().basicType != BasicValType.VTP_INT) {
        return false;
      }
      if (reg2Type.get().basicType != BasicValType.VTP_REAL
          && reg2Type.get().basicType != BasicValType.VTP_INT) {
        return false;
      }
      ...
      if (oper == OpCode.OP_OP_TIMES) {
        if (regType.get().arrayLevel == 0 && reg2Type.get().arrayLevel == 1) { // Vector * scalar
          ...
          freeTempData.set(true);
          return true;
        } else if (regType.get().arrayLevel == 1
            && reg2Type.get().arrayLevel == 0) { // Scalar * vector
        ...
        if (regType.get().arrayLevel == 0 && reg2Type.get().arrayLevel == 2) { // Vector * scalar
          ...
          freeTempData.set(true);
          return true;
        } else if (regType.get().arrayLevel == 2
            && reg2Type.get().arrayLevel == 0) { // Scalar * vector
          operFunction.set(scaleMatrix2);
          resultType.get().setType(regType.get());
          freeTempData.set(true);
          return true;
        } else if (reg2Type.get().arrayLevel == 2
            && regType.get().arrayLevel == 1) { // Matrix * vector
          operFunction.set(matrixVec);
          resultType.get().setType(regType.get());
          freeTempData.set(true);
          return true;
        } else if (reg2Type.get().arrayLevel == 2
            && regType.get().arrayLevel == 2) { // Matrix * matrix
          operFunction.set(matrixMatrix);
          resultType.get().setType(regType.get());
          freeTempData.set(true);
          return true;
        } else if (regType.get().arrayLevel == 1
            && reg2Type.get().arrayLevel == 1) { // Vec * Vec (Dot product)
        ...
        if (regType.get().arrayLevel == 1 && reg2Type.get().arrayLevel == 1) { // Vector + vector
          ...
          return true;
            ...
            && reg2Type.get().arrayLevel == 2) { // Matrix + matrix
