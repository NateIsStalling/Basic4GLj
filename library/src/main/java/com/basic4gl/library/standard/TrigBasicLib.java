...
public class TrigBasicLib implements FunctionLibrary {
    ...
    public Map<String, FunctionSpecification[]> specs() {
        ...
        s.put("Vec4", new FunctionSpecification[] {
            new FunctionSpecification(
                    WrapVec4.class,
                    new ParamTypeList(VTP_REAL, VTP_REAL, VTP_REAL, VTP_REAL),
                    true,
                    true,
        ...
        s.put("MatrixRotate", new FunctionSpecification[] {
            new FunctionSpecification(
                    ...
                    new ParamTypeList(new ValType(VTP_REAL), new ValType(VTP_REAL, (byte) 1, (byte) 1, true)),
                    true,
                    true,
                    new ValType(VTP_REAL, (byte) 2, (byte) 1, true),
                    false,
        ...
        s.put("operator*", new FunctionSpecification[] {
            new FunctionSpecification(
                    WrapOpVecTimesScalar.class,
                    new ParamTypeList(new ValType(VTP_REAL, (byte) 1, (byte) 1, true), new ValType(VTP_REAL)),
                    true,
                    true,
                    new ValType(VTP_REAL, (byte) 1, (byte) 1, true),
                    false,
            ...
            new FunctionSpecification(
                    ...
                    null),
            new FunctionSpecification(
                    WrapOpMatrixTimesScalar.class,
                    new ParamTypeList(new ValType(VTP_REAL, (byte) 2, (byte) 1, true), new ValType(VTP_REAL)),
                    true,
                    true,
                    new ValType(VTP_REAL, (byte) 2, (byte) 1, true),
                    false,
                    ...
                    null),
            new FunctionSpecification(
                    WrapOpScalarTimesMatrix.class,
                    new ParamTypeList(new ValType(VTP_REAL), new ValType(VTP_REAL, (byte) 2, (byte) 1, true)),
                    true,
                    true,
                    new ValType(VTP_REAL, (byte) 2, (byte) 1, true),
                    false,
    ...
    public static float[] getGlobalMatrix() {
        return matrix;
    }
    ...
    public static void rotateAxis(float ang, int main, int a1, int a2) {
        clearMatrix();
        float cosa = (float) Math.cos(ang * Standard.M_DEG2RAD), sina = (float) Math.sin(ang * Standard.M_DEG2RAD);
        matrix[15] = 1;
    ...
    public static void crossProduct(float[] v1, int offset1, float[] v2, int offset2, float[] result, int offset3) {
        ...
        // Calculate w
        result[3 + offset3] =
                calcW(Arrays.copyOfRange(v1, offset1, v1.length), Arrays.copyOfRange(v2, offset2, v2.length));
    }
    ...
    public static float determinant(float[] m) {
        ...
        float res = 0;
        for (int y = 0; y < 4; y++) {
            res += m[y] * m[4 + ((y + 1) & 3)] * m[8 + ((y + 2) & 3)] * m[12 + ((y + 3) & 3)];
            res -= m[y] * m[4 + ((y - 1) & 3)] * m[8 + ((y - 2) & 3)] * m[12 + ((y - 3) & 3)];
        }
        return res;
    }
    ...
    public static void rotateAxis(float ang, float[] v) {
        ...
        // Construct matrix
        clearMatrix();
        matrix[0] = (1 - c) * N[0] * N[0] + c;
        matrix[4] = (1 - c) * N[0] * N[1] - s * N[2];
        matrix[8] = (1 - c) * N[0] * N[2] + s * N[1];
        matrix[1] = (1 - c) * N[0] * N[1] + s * N[2];
        matrix[5] = (1 - c) * N[1] * N[1] + c;
        matrix[9] = (1 - c) * N[1] * N[2] - s * N[0];
        matrix[2] = (1 - c) * N[0] * N[2] - s * N[1];
        matrix[6] = (1 - c) * N[1] * N[2] + s * N[0];
        matrix[10] = (1 - c) * N[2] * N[2] + c;
        matrix[15] = 1;
    }

    static void returnMatrix(VM vm) {
     ...
     */
    static int readVec(VM vm, int index, float[] v) {
        assertTrue(v != null);
     ...
     * @return
     */
    static boolean readMatrix(VM vm, int index, float[] m) {
        assertTrue(m != null);
    ...
    public static final class WrapMatrixZero implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixIdentity implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixScale implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixScale_2 implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixTranslate implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixRotateX implements Function {
        public void run(VM vm) {
            rotateX(vm.getRealParam(1));
            returnMatrix(vm);
        }
    }

    public static final class WrapMatrixRotateY implements Function {
        public void run(VM vm) {
            rotateY(vm.getRealParam(1));
            returnMatrix(vm);
        }
    }

    public static final class WrapMatrixRotateZ implements Function {
        public void run(VM vm) {
            rotateZ(vm.getRealParam(1));
            returnMatrix(vm);
        }
    }

    public static final class WrapMatrixRotate implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixBasis implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapMatrixCrossProduct implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapCross implements Function {
        public void run(VM vm) {

            // Fetch vectors
            int s1 = readVec(vm, vm.getIntParam(2), v1), s2 = readVec(vm, vm.getIntParam(1), v2);
            if (s1 < 0 || s2 < 0) {
                return;
            }
            ...
            vm.setRegIntVal(
                    Data.fillTempRealArray(vm.getData(), vm.getDataTypes(), Math.max(Math.max(s1, s2), 3), result));
        }
    }

    public static final class WrapLength implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapNormalize implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapDeterminant implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapTranspose implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapRTInvert implements Function {
        public void run(VM vm) {
    ...
    public static final class WrapOrthonormalize implements Function {
        public void run(VM vm) {
    ...
    static void doScaleVec(VM vm, float scale, int vecIndex) {
    ...
    static void doScaleMatrix(VM vm, float scale, int matrixIndex) {
    ...
    public final class OpDivVec implements Function {
        public void run(VM vm) {
            doScaleVec(vm, (float) (1.0 / vm.getRegFloatValue()), vm.getReg2IntVal());
        }
    }
    ...
    public final class OpDivMatrix implements Function {
        public void run(VM vm) {
            doScaleMatrix(vm, (float) (1.0 / vm.getRegFloatValue()), vm.getReg2IntVal());
        }
    }

    public static final class OpMatrixVec implements Function {
        public void run(VM vm) {
    ...
    public static final class OpMatrixMatrix implements Function {
        public void run(VM vm) {

            // Matrix * Matrix
            // Left matrix at reg2, right matrix at reg1
            if (!readMatrix(vm, vm.getReg2IntVal(), m1) || !readMatrix(vm, vm.getRegIntVal(), m2)) {
                return;
            }
    ...
    public static final class OpVecVec implements Function {
        public void run(VM vm) {

            // Vector * Vector = dot product

            // Fetch vectors
            if (readVec(vm, vm.getReg2IntVal(), v1) < 0 || readVec(vm, vm.getRegIntVal(), v2) < 0) {
                return;
            }
    ...
    public static final class OpVecPlusVec implements Function {
        public void run(VM vm) {

            // Fetch vectors
            int s1 = readVec(vm, vm.getReg2IntVal(), v1), s2 = readVec(vm, vm.getRegIntVal(), v2);
            if (s1 < 0 || s2 < 0) {
                return;
            }
    ...
    public static final class OpVecMinusVec implements Function {
        public void run(VM vm) {
    ...
    public static final class OpMatrixPlusMatrix implements Function {
        public void run(VM vm) {
    ...
    public static final class OpMatrixMinusMatrix implements Function {
        public void run(VM vm) {
