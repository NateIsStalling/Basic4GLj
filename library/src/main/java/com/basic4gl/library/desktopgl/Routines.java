...
public static int readArrayDynamic(
	...
	case GL11.GL_SHORT:
		return Data.readAndZero(
			vm.getData(), vm.getIntParam(paramIndex), type, array.asShortBuffer(), maxSize);
	case GL11.GL_UNSIGNED_SHORT:
		return Data.readAndZero(
			vm.getData(), vm.getIntParam(paramIndex), type, array.asShortBuffer(), maxSize);
	case GL11.GL_INT:
		return Data.readAndZero(
			vm.getData(), vm.getIntParam(paramIndex), type, array.asIntBuffer(), maxSize);
	case GL11.GL_UNSIGNED_INT:
		return Data.readAndZero(
			vm.getData(), vm.getIntParam(paramIndex), type, array.asIntBuffer(), maxSize);
	case GL11.GL_FLOAT:
		return Data.readAndZero(
			vm.getData(), vm.getIntParam(paramIndex), type, array.asFloatBuffer(), maxSize);
