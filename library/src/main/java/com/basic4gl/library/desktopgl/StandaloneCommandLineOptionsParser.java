...
public class StandaloneCommandLineOptionsParser implements ITargetCommandLineOptions {
    ...
    public StandaloneCommandLineOptions parse(String[] args) {
        ...
        try {
            ...
            result.stateFile = cmd.getOptionValue(stateFileOption, "/" + STATE_FILE);
            result.configFile = cmd.getOptionValue(configFileOption, "/" + CONFIG_FILE);
            ...
            result.displayHelp = cmd.hasOption(help);
            result.displayVersion = cmd.hasOption(version);

            // Args not recognized may be accessed by function libraries; eg: Standard Library args(),
            // argcount()
            result.programArgs = cmd.getArgs();
