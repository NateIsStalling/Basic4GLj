...
public class GLTextGridWindow extends GLWindow implements IFileAccess, ITargetCommandLineOptions {
  ...
  public static void main(String[] args) {
    ...
    boolean isSafeModeEnabled =
        true; // default safe mode to true (only applied if isStandalone=false)
    ...
    try {
      ...
      Option logFileOption =
          Option.builder(LOG_FILE_PATH_OPTION)
              .argName("file")
              .hasArg()
      ...
      Option configFileOption =
          Option.builder(CONFIG_FILE_PATH_OPTION)
              .longOpt("config-path")
              .argName("file")
              .hasArg()
      ...
      Option stateFileOption =
          Option.builder(PROGRAM_FILE_PATH_OPTION)
      ...
      Option mappingFileOption =
          Option.builder(LINE_MAPPING_FILE_PATH_OPTION)
      ...
      Option parentPathOption =
          Option.builder(PARENT_DIRECTORY_OPTION)
              .argName("directory")
              .hasArg()
              .desc("use given directory path for program parent directory")
              .build();

      Option debugPortOption =
          Option.builder(DEBUGGER_PORT_OPTION)
    ...
    for (Library lib : instance.mLibraries) {
      lib.init(
          instance.mComp, instance.mServices); // Allow libraries to register function overloads
    ...
    if (debugServerPort != null) {
      ...
      instance.debuggerAdapter =
          new DebuggerCommandAdapter(
              instance.mMessage, debugger, instance, instance.mComp, instance.mVM);

      instance.debuggerAdapter.connect(debugServerUri);

      instance.mDebugger =
          new DebuggerCallbacks(
              instance.debuggerAdapter, instance.mMessage, instance.mVM, instance) {
            @Override
    ...
    try {
      ...
      if (mDebugger != null) {
        ...
        DebuggerCallbackMessage message =
            new DebuggerCallbackMessage(
                success,
                success == CallbackMessage.SUCCESS ? "Program completed" : mVM.getError(),
                vmStatus);
  ...
  public Configuration getSettings() {
    ...
    settings.addSetting(
        new String[] {"Window Title"}, Configuration.PARAM_STRING, "My Application");
    ...
    settings.addSetting(
        ...
        "0");
    settings.addSetting(
        new String[] {"Mac Support", "32/64-bit", "Do not support"},
        Configuration.PARAM_CHOICE,
        "0");
    settings.addSetting(
        new String[] {"Linux Support", "32/64-bit", "Do not support"},
        Configuration.PARAM_CHOICE,
        "0");
  ...
  private void init() {
    synchronized (GLTextGridWindow.this) {
      ...
      */
      ...
      glfwWindowHint(
          GLFW_RESIZABLE, resizable ? GL_TRUE : GL_FALSE); // the window will be resizable
                ...
                public void invoke(long window, int key, int scancode, int action, int mods) {
                  if (key == GLFW_KEY_ESCAPE && action == GLFW_RELEASE) {
                    glfwSetWindowShouldClose(
                        window, true); // We will detect this in our rendering loop
                  }
                  if (key < 0) {
                    return;
                  }
      ...
      // Center our window
      glfwSetWindowPos(
          m_window, (vidmode.width() - m_width) / 2, (vidmode.height() - m_height) / 2);
