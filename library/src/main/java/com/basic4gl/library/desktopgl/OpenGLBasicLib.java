...
public class OpenGLBasicLib implements FunctionLibrary, IGLRenderer {
  ...
  public Map<String, Constant> constants() {
    ...
    c.put(
        "GL_CLIENT_ACTIVE_TEXTURE_ARB", new Constant(ARBMultitexture.GL_CLIENT_ACTIVE_TEXTURE_ARB));
  ...
  static void loadImageStrip(
      TomVM vm, String filename, int frameWidth, int frameHeight, boolean mipmap) {
    ...
    if (image != null) {
      ...
      OpenGLBasicLib.calculateImageStripFrames(
          image, frameWidth, frameHeight, frameCount, width, height);
      ...
      if (frameCount.get(0) > 0) {
        ...
        vm.getReg()
            .setIntVal(
                Data.fillTempIntArray(vm.getData(), vm.getDataTypes(), frameCount.get(0), t));
        return;
      }
    }
    ...
    vm.getReg()
        .setIntVal(
            Data.fillTempIntArray(vm.getData(), vm.getDataTypes(), 1, new int[] {blankFrame}));
  }
  ...
  static int uploadTexture(Image image) {
    ...
    if (doMipmap) {
      ...
      // Upload texture
      glTexImage2D(
          GL_TEXTURE_2D, 0, image.getFormat(), width, height, 0, format, GL_UNSIGNED_BYTE, pixels);
    }
  ...
  static void internalWrapglTexImage2D(
      TomVM vm, ValType elementType, int dimensions, boolean mipmap) {
    ...
    Routines.readArrayDynamic(
        vm,
        1,
        new ValType(elementType.basicType, (byte) dimensions, (byte) 1, true),
        type,
        data,
        maxSize);
        ...
        ByteBuffer buffer =
            BufferUtils.createByteBuffer(
                size
                    * size
                    * image
                        .getBPP()); // TODO see if 4 should be used instead of image.getBPP () here
          ...
          for (int x = 0; x < width.get(0) / size; x++) {
            ...
            if (mipmap) {
              ...
              */
              ...
              glTexImage2D(
                  GL_TEXTURE_2D, 0, format, size, size, 0, format, GL_UNSIGNED_BYTE, buffer);
              glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
              glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
            }
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
  ...
  public static final class WrapLoadTexStrip implements Function {
    public void run(TomVM vm) {
      ...
      if (texs.size() > 0) {
        ...
        vm.getReg()
            .setIntVal(Data.fillTempIntArray(vm.getData(), vm.getDataTypes(), texs.size(), array));
      } else {
        int[] array = new int[1];
        array[0] = 0;
  ...
  public static final class WrapLoadTexStrip2 implements Function {
    public void run(TomVM vm) {
      ...
      if (texs.size() > 0) {
        ...
        vm.getReg()
            .setIntVal(
                Data.fillTempIntArray(vm.getData(), vm.getDataTypes(), 1, Arrays.asList(array)));
      }
      glPopAttrib();
    }
  }
  ...
  public static final class WrapTexStripFrames2 implements Function {
    public void run(TomVM vm) {
      vm.getReg()
          .setIntVal(
              OpenGLBasicLib.getTexStripFrames(
  ...
  public static final class WrapglGenTextures implements Function {
    public void run(TomVM vm) {
      ...
      handleBuffer.get(t);
      Data.writeArray(
          vm.getData(), vm.getIntParam(1), new ValType(BasicValType.VTP_INT, (byte) 1), t, count);
    }
  }
  ...
  public static final class WrapglDeleteTextures implements Function {
    public void run(TomVM vm) {
      ...
      Data.readAndZero(
          vm.getData(),
          vm.getIntParam(1),
          new ValType(BasicValType.VTP_INT, (byte) 1),
          array,
          count);
  ...
  public static final class WrapLoadImageStrip implements Function {
    public void run(TomVM vm) {
      OpenGLBasicLib.loadImageStrip(
          vm, vm.getStringParam(3), vm.getIntParam(2), vm.getIntParam(1), false);
    }
  }
  ...
  public static final class WrapLoadMipmapImageStrip implements Function {
    public void run(TomVM vm) {
      OpenGLBasicLib.loadImageStrip(
          vm, vm.getStringParam(3), vm.getIntParam(2), vm.getIntParam(1), true);
    }
  }
  ...
  public static final class WrapImageStripFrames implements Function {
    public void run(TomVM vm) {
      vm.getReg()
          .setIntVal(
              OpenGLBasicLib.imageStripFrames(
