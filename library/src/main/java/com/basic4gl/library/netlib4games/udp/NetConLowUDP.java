...
public class NetConLowUDP extends NetConLow implements Runnable {
     ...
     * @param listen
     */
    NetConLowUDP(DatagramChannel sharedSocket, InetSocketAddress addr, int maxPacketSize, NetListenLowUDP listen) {
        super();
    ...
    public void run() {
        ...
        while (!m_socketServiceThread.isTerminating()) {
            ...
            if (!connected) {
                ...
                ThreadEvent[] events = new ThreadEvent[] {m_connectedEvent, m_socketServiceThread.getTerminateEvent()};
    ...
    public boolean connect(String address) {
        ...
        synchronized (serviceLock) {
            if (isClient() && !isConnected()) {
                ...
                // Create socket
                try {
                    m_socket = DatagramChannel.open(StandardProtocolFamily.INET)
