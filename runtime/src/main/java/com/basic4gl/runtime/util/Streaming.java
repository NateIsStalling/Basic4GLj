...
public class Streaming {
	...
	public static void writeLong(DataOutputStream stream, long l) throws IOException {
		// stream.write(ByteBuffer.allocate(Long.SIZE /
		// Byte.SIZE).order(ByteOrder.LITTLE_ENDIAN).putLong(l).array());
		stream.writeLong(l);
	}

	public static void writeShort(DataOutputStream stream, short s) throws IOException {
		// stream.write(ByteBuffer.allocate(Short.SIZE /
		// Byte.SIZE).order(ByteOrder.LITTLE_ENDIAN).putLong(s).array());
		stream.writeShort(s);
	}
	...
	public static void writeFloat(DataOutputStream stream, float f) throws IOException {
		// stream.write(ByteBuffer.allocate(Float.SIZE /
		// Byte.SIZE).order(ByteOrder.LITTLE_ENDIAN).putFloat(f).array());
		stream.writeFloat(f);
	}
