...
public class TomVM extends HasErrorState implements Streamable {
            ...
            switch (instruction.opCode) {
                ...
                case OpCode.OP_ARRAY_INDEX:
                    if (getReg2().getIntVal() != 0) {
                        ...
                        if (getReg().getIntVal() >= 0
                                && getReg().getIntVal()
                                        < mData.data()
                                                .get(getReg2().getIntVal())
                                                .getIntVal()) {
                            assertTrue(
                                    mData.data().get(getReg2().getIntVal() + 1).getIntVal() >= 0);
                ...
                case OpCode.OP_CREATE_USER_FRAME: {
                    ...
                    stackFrame.initForUserFunction(
                            mUserFunctionPrototypes.get(mUserFunctions.get(funcIndex).mPrototypeIndex), funcIndex);
                ...
                case OpCode.OP_RETURN_USER_FUNC: {
                    ...
                    mData.restoreState(stackFrame.prevStackTop, stackFrame.prevTempDataLock, doFreeTempData);
                ...
                case OpCode.OP_COPY_USER_STACK: {
                    ...
                    if (copyToParam(getReg().getIntVal(), mTypeSet.getValType(instruction.value.getIntVal()))) {
                        mIp++; // Proceed to next instruction
                        continue step;
                    } else {
                        break;
                ...
                case OpCode.OP_MOVE_TEMP: {
                    if (moveToTemp(getReg().getIntVal(), mTypeSet.getValType(instruction.value.getIntVal()))) {
                        mIp++; // Proceed to next instruction
                        continue step;
                    } else {
                        break;
    ...
    void copyArray(int sourceIndex, int destIndex, ValType type) {
        ...
        assertTrue(mData.isIndexValid(sourceIndex));
        assertTrue(mData.isIndexValid(destIndex));
        assertTrue(mData.data().get(sourceIndex).getIntVal()
                == mData.data().get(destIndex).getIntVal()); // Array sizes match
        assertTrue(mData.data().get(sourceIndex + 1).getIntVal()
    ...
    void copyField(int sourceIndex, int destIndex, ValType type) {
        ...
        if (type.matchesType(BasicValType.VTP_STRING)) {
            ...
            if (src.getIntVal() > 0 || dest.getIntVal() > 0) {
                ...
                mStrings.setValue(
                        dest.getIntVal(),
                        mStrings.getValueAt(mData.data().get(sourceIndex).getIntVal()));
            }
        }
    ...
    boolean copyData(int sourceIndex, int destIndex, ValType type) {
        ...
        if (!mData.isIndexValid(sourceIndex) || !mData.isIndexValid(destIndex) || sourceIndex == 0 || destIndex == 0) {
            setError(ERR_UNSET_POINTER);
            return false;
        }
