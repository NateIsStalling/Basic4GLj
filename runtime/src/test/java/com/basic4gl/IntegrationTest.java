...
public class IntegrationTest {
	...
	void compilesDimWithMultipleVariables() {
		...
		assertCodeCompiles(program);
		assertTrue(this.vm.getVariables().containsVariable("a"),
				"Variable should be created in the VM.");
		assertTrue(this.vm.getVariables().containsVariable("b$"),
				"Variable should be created in the VM.");
		assertTrue(this.vm.getVariables().containsVariable("c#"),
				"Variable should be created in the VM.");
		assertBytecodeOutput(new Instruction[] {
	...
	Value findGlobalVariable(String globalName) {
		...
		assertTrue(vars.containsVariable(globalName),
	...
	void assertBytecodeOutput(Instruction[] expected) {
		...
		for (; i < expected.length; i++) {
			...
			assertEquals(expected[i].basicVarType, actual[i].basicVarType,
					"BasicVarType differ at index " + i + ", expected "
							+ expected[i].basicVarType + " but got " + actual[i].basicVarType);
