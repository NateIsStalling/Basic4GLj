...
private final Map<String, Operator>
	unaryOperators; // Recognised operators. Unary have one operand (e.g NOT x)
...
private final Vector<BinaryOperatorExtension>
	binaryOperatorExtensions; // Binary operator extensions
...
public TomBasicCompiler(TomVM vm, boolean caseSensitive) {
	...
	binaryOperators.put(
		"land", new Operator(OperType.OT_LAZYBOOLOPERATOR, OpCode.OP_OP_AND, 2, 12));
	unaryOperators.put("not", new Operator(OperType.OT_BOOLOPERATOR, OpCode.OP_OP_NOT, 1, 20));
	binaryOperators.put(
		"=", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_EQUAL, 2, 30));
	binaryOperators.put(
		"<>", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_NOT_EQUAL, 2, 30));
	binaryOperators.put(
		">", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_GREATER, 2, 30));
	binaryOperators.put(
		">=", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_GREATER_EQUAL, 2, 30));
	binaryOperators.put(
		"<", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_LESS, 2, 30));
	binaryOperators.put(
		"<=", new Operator(OperType.OT_RETURNBOOLOPERATOR, OpCode.OP_OP_LESS_EQUAL, 2, 30));
...
boolean needAutoEndif() {
	...
	// "else"
	return (top.controlType == FlowControlType.FCT_IF
			|| top.controlType == FlowControlType.FCT_ELSE)
		&& !top.blockIf;
}
...
UserFuncPrototype getCurrentUserFunctionPrototype() {
	...
	assertTrue(getCurrentUserFunction().mPrototypeIndex < vm.getUserFunctionPrototypes().size());
...
public void rollback(RollbackPoint rollbackPoint) {
	...
	for (Iterator<Map.Entry<String, Integer>> it = globalUserFunctionIndex.entrySet().iterator();
		it.hasNext(); ) {
	...
	for (Iterator<Map.Entry<Integer, String>> it = userFunctionReverseIndex.entrySet().iterator();
		it.hasNext(); ) {
	...
	for (Iterator<Map.Entry<String, Integer>> it = runtimeFunctionIndex.entrySet().iterator();
		it.hasNext(); ) {
	Map.Entry<String, Integer> entry = it.next();
...
private boolean compileDim(boolean forStruc, boolean forFuncParam) {
	...
	if (forStruc) {
		...
		if (varIndex >= 0) {
			...
			setError(
				"Local variable '" + name + "' has already been allocated as a different type.");
			return false;
			}
		...
		if (varIndex >= 0) {
			if (!(vm.getVariables().getVariables().get(varIndex).type.equals(type))) {
...
private boolean compileDataType(
	Mutable<String> name, Mutable<ValType> type, Mutable<TokenType> tokenType) {
	...
	if (last == '$' || last == '#' || last == '%') {
		setError(
			(String) "\"" + name + "\" is a structure variable, and cannot end with #, $ or %");
		return false;
	}
	}
...
private boolean compileAs(Mutable<String> name, Mutable<ValType> type) {
	...
	if (token.getTokenType() != TokenType.CTT_TEXT
		&& token.getTokenType() != TokenType.CTT_KEYWORD) {
...
private boolean compileExpression(boolean mustBeConstant) {
	...
	// on the stack.
	operatorStack.add(
		new StackedOperator(new Operator(OperType.OT_STOP, OpCode.OP_NOP, 0, -200000))); // Stop
	// evaluation
	// operator
...
boolean internalCompileAssignment() {
	...
	if (regType.isNull()
		|| (regType.arrayLevel == reg2Type.arrayLevel
...
private boolean compileIf(boolean elseif) {
	...
	// end of the line
	boolean autoEndif =
		(syntax == LanguageSyntax.LS_TRADITIONAL) // Only
			// applies
			// to
			// traditional syntax
...
private boolean compileEndIf(boolean automatic) {
	...
	if (!(isFlowControlTopEqual(FlowControlType.FCT_IF)
		|| isFlowControlTopEqual(FlowControlType.FCT_ELSE))) {
...
private boolean compileDo() {
	...
	flowControls.add(
		new FlowControl(
			FlowControlType.FCT_DO_POST, vm.getInstructionCount(), loopPos, line, col));
	return true;
	}
}
...
private boolean compileFunction(boolean needResult) {
	...
	ExtendedFunctionSpecification[] functions =
		new ExtendedFunctionSpecification[TC_MAXOVERLOADEDFUNCTIONS];
	int functionCount = 0;
	...
	for (i = 0; i < functionCount && matchIndex < 0; i++) {
		ValType type = new ValType(functions[i].spec.getParamTypes().getParams().get(count));
	...
	if (matchIndex >= 0) {
		...
		ValType type =
			new ValType(functions[matchIndex].spec.getParamTypes().getParams().get(count));
		...
		if (isAnyType) {
			...
			// that
			// same type.
			if (functions[src].spec.getParamTypes().getParams().get(count).equals(type)) {
			functions[dst++] = functions[src];
			}
		}
		}
...
private boolean compileExtendedUnOperation(short operOpCode) {
	...
	// Call function
	found =
		unaryOperatorExtensions.get(i).run(type, operOpCode, opFunc, resultType, freeTempData);
	}
...
private boolean compileExtendedBinOperation(short operOpCode) {
	...
	found =
		binaryOperatorExtensions
			.get(i)
			.run(type1, type2, operOpCode, opFunc, resultType, freeTempData);
	}
...
private boolean compileData() {
	...
	if (token.getText().equals(",") || atSeparatorOrSpecial()) {
		...
		// Allocate new string constant
		String text =
			token.getText().substring(1, token.getText().length() - 1); // Remove S prefix
...
private boolean compileUserFunction(UserFunctionType funcType) {
	...
	while (token.getText().equals("(")) {
		...
		setError(
			(String) "Arrays cannot have more than "
				+ String.valueOf(TomVM.ARRAY_MAX_DIMENSIONS)
				+ " dimensions.");
		return false;
		}
	...
	if (isRuntimeFunction(name)) {
		setError(
			(String) "'" + name + "' has already been used as a runtime function/subroutine name");
		return false;
	}
	...
	if (isRuntimeFunction(name)) {
		...
		if (!userFuncPrototype.matches(
			prototypes.get(functions.get(currentFunction).mPrototypeIndex))) {
...
public boolean streamIn(DataInputStream stream) {
	...
	for (int i = 0; i < count; i++) {
		com.basic4gl.compiler.RuntimeFunction function =
			new com.basic4gl.compiler.RuntimeFunction();
