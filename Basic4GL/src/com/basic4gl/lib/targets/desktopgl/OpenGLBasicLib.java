...
import com.basic4gl.vm.util.IntHandleResources;
import com.basic4gl.vm.util.PointerResourceStore;
    ...
    static void LoadImageStrip (
        ...
        if (image != null) {
            ...
            if (frameCount.get(0) > 0) {
                ...
                ByteBuffer tex = ByteBuffer.allocate(Integer.SIZE/Byte.SIZE * 65536);
                glGenTextures (frameCount.get(0), tex);
...
// Helper functions
    ...
    static int TexStripFrames(String filename){ return OpenGLBasicLib.TexStripFrames(filename, 0, 0);}
...
// Function wrappers
    ...
    static void InternalWrapglTexImage2D(TomVM vm, ValType elementType, int dimensions, boolean mipmap) {
        ...
        ByteBuffer data = ByteBuffer.allocate(maxSize * 4);
    ...
    public final class WrapglGenTextures  implements Function {
        public void run(TomVM vm) {
            ...
            for (int i = 0; i < count; i++)
                OpenGLBasicLib.textures.Store (handles.asIntBuffer().get(i));

            // Store handles in Basic4GL array
            int t[] = new int[handles.asIntBuffer().capacity()];
            handles.asIntBuffer().get(t);
    ...
    public final class WrapglDeleteTextures  implements Function {
        public void run(TomVM vm) {
            ...
            ByteBuffer handles = BufferUtils.createByteBuffer((Integer.SIZE/Byte.SIZE) * 65536);            // 64K should be enough for anybody ;)
            Data.ReadAndZero(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_INT, (byte)1), handles.asIntBuffer().array(), count);
    ...
    public final class WrapglLoadMatrixd implements Function {
        public void run(TomVM vm) {
            ...
            glLoadMatrixd(a1);Data.WriteArray(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte) 2, (byte) 1, true),a1.array(), 16);
        }
    }
    public final class WrapglLoadMatrixf implements Function {
        public void run(TomVM vm) {
            ...
            Data.ReadAndZero(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte) 2, (byte) 1, true), a1.array(), 16);
            glLoadMatrixf(a1);Data.WriteArray(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte) 2, (byte) 1, true), a1.array(), 16);
        }
    }
    public final class WrapglMultMatrixd implements Function {
        public void run(TomVM vm) {
            DoubleBuffer a1 = BufferUtils.createDoubleBuffer(16);
            Data.ReadAndZero(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte) 2, (byte) 1, true), a1.array(), 16);
            glMultMatrixd(a1);Data.WriteArray(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte) 2, (byte) 1, true), a1.array(), 16);
        }
    }
    public final class WrapglMultMatrixf implements Function {
        public void run(TomVM vm) {
            FloatBuffer a1 = BufferUtils.createFloatBuffer(16);
            Data.ReadAndZero(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte)2,(byte)1, true), a1.array(), 16);
            glMultMatrixf(a1);Data.WriteArray(vm.Data(), vm.GetIntParam(1), new ValType(ValType.VTP_REAL, (byte)2, (byte)1, true), a1.array(), 16);
        }
    }
...
// them for backwards compatibility.
    ...
    void OldSquare_LoadImageStrip (TomVM vm, String filename, IntBuffer frameSize, boolean mipmap) {
        ...
        Image image = LoadImage.LoadImage(filename);
        if (image != null) {
            IntBuffer frameCount = IntBuffer.allocate(1), width = IntBuffer.allocate(1), height = IntBuffer.allocate(1);
                        ...
                        if (mipmap) {
                            ...
                            */
                            GL11.glTexImage2D(GL11.GL_TEXTURE_2D,
                                    0,
                                    image.getFormat(),
                                    frameSize.get(0), frameSize.get(0), 0,
                                    format, GL11.GL_UNSIGNED_BYTE,
                                    buffer);
                            GL30.glGenerateMipmap(GL11.GL_TEXTURE_2D);
                            glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
