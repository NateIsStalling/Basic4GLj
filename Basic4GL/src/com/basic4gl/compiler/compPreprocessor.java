...
public class compPreprocessor extends HasErrorState {
    ...
    public compPreprocessor(int serverCount, ISourceFileServer ... server){
        // Register source file servers
        for (int i = 0; i < serverCount; i++)
            fileServers.add(server[i]);
    }
    protected void finalize() //virtual ~compPreprocessor();
    {
    ...
    public boolean Preprocess(ISourceFile mainFile, compParser parser)
    {
    ...
    /// Member access
    public CompLineNumberMapping LineNumberMap() { return lineNumberMap; }
}
