...
public class EmptyTabPanel extends JPanel {
    ...
    private JPanel buildGetStartedPanel(IEmptyTabPanelListener listener, KeyStroke newFileKeyStroke, KeyStroke openFileKeyStroke) {
    ...
    private JPanel buildRecentFilesPanel(IEmptyTabPanelListener listener, List<File> recentFiles){
        ...
        for (File file:
                recentFiles.stream().limit(5).toList()) {
            ...
            recentItemButton.addActionListener((x) -> {
    ...
    private void addShortCutLabel(JPanel parent, KeyStroke keyStroke) {
        for (String s:
                KeyStrokeUtil.getShortcutString(keyStroke).split(" ")) {
            ...
            if (!s.equals("+")) {
                ...
                l.putClientProperty("JComponent.padding", new Insets(10, 15, 10, 15)); // Adds internal padding
