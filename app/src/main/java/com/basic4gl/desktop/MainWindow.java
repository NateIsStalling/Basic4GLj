...
public class MainWindow
  ...
  private final CaretListener TrackCaretPosition =
      new CaretListener() {
        @Override
  ...
  public static void main(String[] args) {
    ...
    if (args.length >= 2) {
      ...
      // builds to include these as config settings
      String appHome =
          System.getenv("APP_HOME"); // APP_HOME is defined in scripts distributed with build
  ...
  public MainWindow() {
    ...
    saveMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_S, toolkit.getMenuShortcutKeyMask()));
    ...
    undoMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_Z, toolkit.getMenuShortcutKeyMask()));
    ...
    redoMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_Y, toolkit.getMenuShortcutKeyMask()));
    ...
    cutMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_X, toolkit.getMenuShortcutKeyMask()));
    ...
    copyMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_C, toolkit.getMenuShortcutKeyMask()));
    ...
    pasteMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_V, toolkit.getMenuShortcutKeyMask()));
    ...
    findMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_F, toolkit.getMenuShortcutKeyMask()));
    ...
    replaceMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(
            KeyEvent.VK_F, InputEvent.SHIFT_MASK | toolkit.getMenuShortcutKeyMask()));
    replaceMenuItem.addActionListener(
        e -> {
          showFindReplaceMenu(true);
        });
    selectAllMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_A, toolkit.getMenuShortcutKeyMask()));
    ...
    nextBookmarkMenuItem.addActionListener(
        e -> {
          int i = tabControl.getSelectedIndex();
          fileManager.selectNextBookmark(i);
        });
    previousBookmarkMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(KeyEvent.VK_F2, InputEvent.SHIFT_MASK));
    previousBookmarkMenuItem.addActionListener(
        e -> {
          int i = tabControl.getSelectedIndex();
    ...
    stepOverMenuItem.addActionListener(
        e -> {
    ...
    toggleBreakpointMenuItem.addActionListener(
        e -> {
          int i = tabControl.getSelectedIndex();
    ...
    functionListMenuItem.addActionListener(
        e -> {
    ...
    Preprocessor preprocessor =
        new Preprocessor(2, new EditorSourceFileServer(fileManager), new DiskFileServer());
    ...
    basicEditor =
        new BasicEditor(
            outputBinPath, fileManager, this, appSettings, preprocessor, debugger, comp);
    ...
    DebugServerFactory.startDebugServer(
        debugServerBinPath, DebugServerConstants.DEFAULT_DEBUG_SERVER_PORT);
  ...
  private void actionExport() {
    ...
    if (fileManager.editorCount() == 0) {
      JOptionPane.showMessageDialog(
          frame, "Nothing to export", "Cannot export", JOptionPane.WARNING_MESSAGE);
      return;
    }
  ...
  public void openTab(String filename) {
    ...
    MainWindow.this.addTab(
        FileEditor.open(file, this, fileManager, this, linkGenerator, searchContext));
  ...
  boolean actionSave() {
    ...
    boolean saved =
        fileManager.getFileEditors().get(index).save(false, fileManager.getCurrentDirectory());
    if (saved) {
  ...
  public void PlaceCursorAtProcessed(final int row, int col) {
    ...
    if (r >= 0) {
      ...
      if (index == -1) {
        // Attempt to open tab
        addTab(
            FileEditor.open(new File(file), this, fileManager, this, linkGenerator, searchContext));
      ...
      SwingUtilities.invokeLater(
          () -> {
            ...
            if (r >= 0) {
              try {
                JTextArea textArea =
                    fileManager.getFileEditors().get(tabControl.getSelectedIndex()).getEditorPane();
  ...
  public void RefreshDebugDisplays(ApMode mode) {
    ...
    if (mode != ApMode.AP_CLOSED) {
      playButton.setIcon(
          mode == ApMode.AP_RUNNING ? createImageIcon(ICON_PAUSE) : createImageIcon(ICON_PLAY));
      playButton.setEnabled(true);
  ...
  public void updateCallStack(StackTraceCallback stackTraceCallback) {
    ...
    int totalFrames =
        stackTraceCallback.stackFrames
            .size(); // callback.totalFrames may be larger if paging is enforced
