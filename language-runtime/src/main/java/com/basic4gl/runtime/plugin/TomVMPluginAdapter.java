...
public class TomVMPluginAdapter implements Basic4GLRuntime {
    ...
    private void ReadIntArray(int dataIndex, int[] dest, int dimensionCount, int[] dimensions) {
        ReadIntArray(dataIndex, dest, 0, dimensionCount, dimensions, 0);
    }

    private void ReadIntArray(
            int dataIndex, int[] dest, int destOffset, int dimensionCount, int[] dimensions, int dimensionOffset) {
        ...
        if (dimensionCount > 1) {
            ...
            int destElements = 1;
            for (int d = 1; d < dimensionCount; d++) destElements *= dimensions[dimensionOffset + d];
            ...
            for (int i = 0; i < dimensions[dimensionOffset]; i++)
                if (i < elementCount) {
                    dest[destOffset + i] =
                            vm.getData().data().get(dataIndex + i).getIntVal();
                } else {
                    dest[destOffset + i] = 0;
                }
        }
    ...
    private void ReadFloatArray(int dataIndex, float[] dest, int dimensionCount, int[] dimensions) {
        ReadFloatArray(dataIndex, dest, 0, dimensionCount, dimensions, 0);
    }

    private void ReadFloatArray(
            int dataIndex, float[] dest, int destOffset, int dimensionCount, int[] dimensions, int dimensionOffset) {
        ...
        if (dimensionCount > 1) {
            ...
            for (int i = 0; i < dimensions[dimensionOffset]; i++)
                if (i < elementCount)
                    dest[destOffset + i] =
                            vm.getData().data().get(dataIndex + i).getRealVal();
                else dest[destOffset + i] = 0;
        }
    }

    private void WriteIntArray(int dataIndex, int[] src, int dimensionCount, int[] dimensions) {
        WriteIntArray(dataIndex, src, 0, dimensionCount, dimensions, 0);
    }

    private void WriteIntArray(
            int dataIndex, int[] src, int srcOffset, int dimensionCount, int[] dimensions, int dimensionOffset) {
    ...
    private void WriteFloatArray(int dataIndex, float[] src, int dimensionCount, int[] dimensions) {
        WriteFloatArray(dataIndex, src, 0, dimensionCount, dimensions, 0);
    }

    private void WriteFloatArray(
            int dataIndex, float[] src, int srcOffset, int dimensionCount, int[] dimensions, int dimensionOffset) {
        ...
        if (dimensionCount > 1) {
            ...
            // Down to lowest dimension.
            // Can copy elements
            for (int i = 0; i < min; i++) vm.getData().data().get(dataIndex + i).setRealVal(src[srcOffset + i]);
        }
    }

    private int CreateTempArray(ValType type, int dimensionCount, int[] dimensions) {

        // Create Basic4GL type to define array
        for (int d = 0; d < dimensionCount; d++) type.addDimension(dimensions[d]);
    ...
    private void FixCurrentType() {
        ...
        if (currentType.getPointerLevel() == 0 && (currentType.getBaseType() > 0 || currentType.getArrayLevel() > 0)) {
    ...
    private void CValueFromBasicValue(PluginDataType type, ByteBuffer cData, Value value) {
        ...
        switch (type.getBaseType()) {
            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_BYTE:
                // Copy Basic4GL int to C byte
                cData.put((byte) value.getIntVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_WORD:
                // Copy Basic4GL int to C word (int16_t)
                cData.putChar((char) value.getIntVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_INT:
                // Copy Basic4GL int to C int
                cData.putInt(value.getIntVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_INT64:
                // Copy Basic4GL int to C 64bit integer (int64_t)
                cData.putLong(value.getIntVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_FLOAT:
                // Copy Basic4GL real to C float
                cData.putFloat(value.getRealVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_DOUBLE:
                // Copy Basic4GL real to C double
                cData.putDouble(value.getRealVal());
                break;

            case com.basic4gl.language.core.extensions.Basic4GLExtendedTypeCode.PLUGIN_BASIC4GL_EXT_STRING:
    ...
    // Array parameters
    public void getIntArrayParam(int index, int[] array, int dimensions, int dimension0Size, int... otherDimensions) {
        ...
        ReadIntArray(vm.getIntParam(index), array, dimensions, dimensionArray);
    }

    public void getFloatArrayParam(
            int index, float[] array, int dimensions, int dimension0Size, int... otherDimensions) {
        Assert.assertTrue(dimensions < ARRAY_MAX_DIMENSIONS);
        int[] dimensionArray = new int[ARRAY_MAX_DIMENSIONS];
        if (dimensions > 0) {
            dimensionArray[0] = dimension0Size;
        ...
        ReadFloatArray(vm.getIntParam(index), array, dimensions, dimensionArray);
    }

    public int getArrayParamDimension(int index, int dimension) {

        return Data.getArrayDimensionSize(vm.getData(), vm.getIntParam(index), dimension);
    }

    // Array results
    public void setIntArrayResult(int[] array, int dimensions, int dimension0Size, int... otherDimensions) {
    ...
    public void setType(int baseType) {

        currentType = PluginDataType.simpleType(baseType);
    }

    public void setStringType(int size) {

        currentType = PluginDataType.string(size);
    }

    public void modTypeArray(int dimensions, int dimension0Size, int... otherDimensions) {
    ...
    public Object getParam(int index, ByteBuffer dst) {
    ...
    public void setReturnValue(ByteBuffer src) {
    ...
    public void directSetString(int memAddr, String str) {
        ...
        if (dest.getIntVal() == 0) dest.setIntVal(vm.allocString());
