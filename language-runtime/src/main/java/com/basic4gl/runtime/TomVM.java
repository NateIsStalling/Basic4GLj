...
public class TomVM extends HasErrorState implements VM, ProgramStreamable {
    ...
    private final ArrayList<com.basic4gl.language.core.stackframe.UserFuncPrototype> userFunctionPrototypes;
    ...
    @Override
    public String getValueString(Variable vmVariable) {
        ...
        if (valueType.isBasicType() || valueType.pointerLevel > 0) {
            value = getData().data().get(vmVariable.dataIndex);
        } else {
        ...
        try {
            ...
            return valToString(value, valueType, maxChars);
        } catch (Exception ex) {
            return ERROR_VALUE;
        }
    }
    ...
    @Override
    public String getDisplayVariable(ValType valType) {
        if (valType.matchesType(BasicValType.VTP_STRING)) { // String is special case.
            return "\"" + getRegString() + "\""; // Stored in string register.
        } else {
            String temp;
            try {
                Mutable<Integer> maxChars = new Mutable<>(TomVM.DATA_TO_STRING_MAX_CHARS);
                temp = valToString(new Value(getRegIntVal()), valType, maxChars);
            } catch (Exception ex) {
            ...
            switch (instructionOpCode) {
                ...
                case OpCode.OP_REG_DESTRUCTOR: {
                    ...
                    if (ptr == 0) {
                        ...
                        assertTrue(tempDestructors.isEmpty() || CollectionUtil.last(tempDestructors).addr < ptr);
                        ...
                        assertTrue(stackDestructors.isEmpty() || CollectionUtil.last(stackDestructors).addr > ptr);
    ...
    public Register evaluateExpression(int expressionStart) {
        ...
        return new Register(new Value(getRegIntVal()), getRegString());
    }
    ...
    public String getOpCodeData(Instruction i, IFunctionIndex functions) {
        ...
        switch (i.opCode) {
            ...
            case OP_CALL_DLL: {
                int index = i.value.getIntVal();
                return getPlugins().getFunctionName(index >> 24, index & 0x00ffffff) + "()";
            }
            case OP_COPY:
            case OP_ALLOC: {
                return getDataTypes().describeVariable("", getStoredType(i.value.getIntVal()));
            }
            case OP_CREATE_USER_FRAME: {
                return functions.getUserFunctionName(i.value.getIntVal()) + "()";
            }
            case OP_LOAD_CONST: {
                if (i.basicVarType == BasicValType.VTP_FUNC_PTR) {
                    if (i.value.getIntVal() == 0) return "[NULL]";
                    return functions.getUserFunctionName(i.value.getIntVal() - 1) + "()";
                }
            }
        }
        return basicValToString(i.value, i.basicVarType, true);
    }
}
