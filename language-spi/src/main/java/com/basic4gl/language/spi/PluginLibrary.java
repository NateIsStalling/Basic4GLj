...
public class PluginLibrary implements com.basic4gl.language.core.extensions.Basic4GLFunctionRegistry {
    ...
    public void modStrucFieldArray(byte dimensions, int dimension1Size, int... otherSizes) {
        ...
        Assert.assertTrue(dimensions < ArrayConstants.ARRAY_MAX_DIMENSIONS);
        int[] dimensionArray = new int[ArrayConstants.ARRAY_MAX_DIMENSIONS];
        if (dimensions > 0) {
    ...
    /**
     * Find constant with a given name within this plugin.
    ...
    /**
     * Return true if name matches a function in this plugin.
    ...
    /**
     * Find all functions of a given name within this plugin and add them to the array.
     * @param name
     * @param functions
     * @param count
     * @param max
     ...
     * @return function if found, or null if not found.
     */
    ...
    /**
     * @return error text if plugin failed to load or initialise, or null if no error.
     */
     ...
     * @param plugin Plugin that no longer references this one
     */
